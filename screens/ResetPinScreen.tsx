...
interface ResetPinScreenProps {
  email: string;
  token: string;                // arriva dal link, lo passiamo a resetPin ma NON lo validiamo lato server
  onResetSuccess: () => void;   // torna al login dopo il reset
}
...
const ResetPinScreen: React.FC<ResetPinScreenProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // Reset locale: nessuna chiamata ad Apps Script
      const res = await resetPin(normalizedEmail, token || '', pin);
      ...
      // Piccolo delay per mostrare il messaggio e poi tornare al login
      setTimeout(() => {
        onResetSuccess();
      }, 800);
    } catch (err) {
