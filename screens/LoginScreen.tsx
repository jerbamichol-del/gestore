...
const LoginScreen: React.FC<LoginScreenProps> = ({
  ...
  // --- MODIFICA: Handler asincrono per gestire il Cloud Restore ---
  const handleEmailSubmit = async (email: string) => {
    if (!email) return;
    const normalized = email.toLowerCase();
    
    // 1. Controllo se esiste nel DB locale
    const localUsers = getUsers();
    
    if (localUsers[normalized]) {
      // Utente trovato in locale -> flusso standard
      setActiveEmail(normalized);
      setError(null);
    ...
    // 2. Utente NON trovato in locale -> Provo il CLOUD (Ripristino)
    setIsLoading(true);
    try {
      const cloudResult = await loadFromCloud(normalized);
      
      if (cloudResult) {
        // TROVATO! Ripristino i dati e l'utente
        
        // A. Ripristino dati spese
        ...
        // B. Ripristino l'UTENTE (Hash e Salt) nel database locale
        // Così l'app sa che esiste e può verificare il PIN
        ...
        // Non trovato neanche nel cloud
        setError("Nessun account trovato (nè locale nè cloud).");
      }
    } catch (e) {
      console.error(e);
      setError("Errore durante il controllo cloud.");
    } finally {
      setIsLoading(false);
    }
  };
