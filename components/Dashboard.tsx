...
recurringExpenses.forEach(template => {
    ...
    // Verifica preliminare: se la spesa programmata è "finita" (es. spese singole già pagate o limite raggiunto),
    // non la contiamo affatto.
    ...
    while (nextDue) {
        ...
        if (template.recurrenceEndType === 'count' && template.recurrenceCount) {
             // Se abbiamo superato il totale teorico (basato sull'indice assoluto dall'inizio), stop.
        ...
        // Nota: includiamo anche quelle generate se il template è ancora attivo (gestito dal check preliminare sopra).
        if (nextDue >= start) {
            recurringCount++;
        }
