...
const getRecurrenceSummary = (expense: Partial<Expense>): string => {
  ...
  if (recurrenceInterval === 1) {
    summary = recurrenceLabels[recurrence as keyof typeof recurrenceLabels];
  } else {
    switch (recurrence) {
...
const TransactionDetailPage = React.forwardRef<HTMLDivElement, TransactionDetailPageProps>(({
  ...
  // --------- IMPORTANTI MICRO-PATCH PER LO SWIPE SU MOBILE ----------
  const panY = { touchAction: 'pan-y' as const, overscrollBehaviorX: 'contain' as const };
  const tapOnly = { touchAction: 'manipulation' as const }; // sui bottoni per non “rubare” il pan-x
  // ------------------------------------------------------------------
  ...
  if (typeof formData.amount !== 'number') {
    return (
      <div
        ...
        style={panY}
      >
        <header className="p-4 flex items-center gap-4 text-slate-800 bg-white shadow-sm absolute top-0 left-0 right-0 z-10" style={tapOnly}>
          {!isDesktop && (
            <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-200" aria-label="Torna alla calcolatrice" style={tapOnly}>
  ...
  return (
    ...
    >
      <header
        className="p-4 flex items-center justify-between gap-4 text-slate-800 bg-white shadow-sm sticky top-0 z-10"
        style={tapOnly}
      >
      ...
      >
        <div className="space-y-4">
          ...
          <div className="bg-white p-4 rounded-lg border border-slate-200 space-y-4" style={panY}>
            <div>
      ...
      <SelectionMenu
        ...
        // Se il tuo SelectionMenu accetta style sul wrapper, assicurati che usi touchAction: 'pan-y'
      />
        ...
        >
          ...
          >
            <div className="flex justify-between items-center p-4 border-b border-slate-200">
              ...
              <button type="button" onClick={handleCloseFrequencyModal} className="text-slate-500 hover:text-slate-800 transition-colors p-1 rounded-full hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500" aria-label="Chiudi" style={tapOnly}>
                <XMarkIcon className="w-6 h-6" />
              </button>
            </div>
            <div className="p-4 space-y-2">
              <button onClick={() => handleFrequencySelect('none')} className="w-full text-center px-4 py-3 text-base font-semibold rounded-lg transition-colors bg-slate-100 text-slate-800 hover:bg-indigo-100 hover:text-indigo-800" style={tapOnly}>Nessuna</button>
              <button onClick={() => handleFrequencySelect('single')} className="w-full text-center px-4 py-3 text-base font-semibold rounded-lg transition-colors bg-slate-100 text-slate-800 hover:bg-indigo-100 hover:text-indigo-800" style={tapOnly}>Singolo</button>
              <button onClick={() => handleFrequencySelect('recurring')} className="w-full text-center px-4 py-3 text-base font-semibold rounded-lg transition-colors bg-slate-100 text-slate-800 hover:bg-indigo-100 hover:text-indigo-800" style={tapOnly}>Ricorrente</button>
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          >
            <header className="flex justify-between items-center p-4 border-b border-slate-200">
              ...
              <button type="button" onClick={handleCloseRecurrenceModal} className="text-slate-500 hover:text-slate-800 transition-colors p-1 rounded-full hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500" aria-label="Chiudi" style={tapOnly}>
                <XMarkIcon className="w-6 h-6" />
              </button>
            </header>

            <main className="p-4 space-y-4" style={panY}>
              <div className="relative">
                <button
                ...
                {isRecurrenceOptionsOpen && (
                  <div className="absolute top-full mt-1 w-full bg-white border border-slate-200 shadow-lg rounded-lg z-20 p-2 space-y-1 animate-fade-in-down" style={panY}>
              ...
              <div className="animate-fade-in-up pt-2" style={{ animationDuration: '200ms' }}>
                <div className="flex items-center justify-center gap-2 bg-slate-100 p-3 rounded-lg">
                  ...
                  <span className="text-base text-slate-700">
                    {(() => {
                      const count = tempRecurrenceInterval || 1;
                      switch (tempRecurrence) {
                        case 'daily': return count === 1 ? 'giorno' : 'giorni';
                        case 'weekly': return count === 1 ? 'settimana' : 'settimane';
                        case 'monthly': return count === 1 ? 'mese' : 'mesi';
                        case 'yearly': return count === 1 ? 'anno' : 'anni';
              ...
              {tempRecurrence === 'weekly' && (
                <div className="animate-fade-in-up pt-2" style={panY}>
                  <div className="flex flex-wrap justify-center gap-2">
                    {daysOfWeekForPicker.map(day => (
                      <button
                        key={day.value}
                        onClick={() => setTempRecurrenceDays(prev => {
                          const curr = prev || [];
                          const next = curr.includes(day.value) ? curr.filter(d => d !== day.value) : [...curr, day.value];
              ...
              {tempRecurrence === 'monthly' && (
                <div className="animate-fade-in-up pt-4 space-y-2 border-t border-slate-200" style={panY}>
                  <div
                    role="radio"
              ...
              <div className="pt-4 border-t border-slate-200" style={panY}>
                <div className="grid grid-cols-2 gap-4 items-end">
                  <div className={`relative ${!formData.recurrenceEndType || formData.recurrenceEndType === 'forever' ? 'col-span-2' : ''}`}>
                    ...
                    >
                      <span className="truncate flex-1 capitalize">
                        {!formData.recurrenceEndType || formData.recurrenceEndType === 'forever'
                          ? 'Per sempre'
                    ...
                    {isRecurrenceEndOptionsOpen && (
                      <div className="absolute top-full mt-1 w-full bg-white border border-slate-200 shadow-lg rounded-lg z-10 p-2 space-y-1 animate-fade-in-down" style={panY}>
            ...
            <footer className="p-4 bg-slate-100 border-t border-slate-200 flex justify-end" style={panY}>
              <button
                type="button"
