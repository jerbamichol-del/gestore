...
const getRecurrenceLabel = (value?: keyof typeof recurrenceLabels) =>
  value ? recurrenceLabels[value] : null;
...
const TransactionDetailPage = React.forwardRef<HTMLDivElement, TransactionDetailPageProps>(
  (
    ...
    // Focus solo quando tocchi quell’input (nessun focus automatico di pagina)
    ...
    return (
      ...
      >
                  ...
                  <div className="grid grid-cols-2 gap-4 items-end">
                    ...
                    {formData.recurrenceEndType === 'date' && (
                      <div className="animate-fade-in-up">
                        <label
                          className="block text-base font-medium text-slate-700 mb-1 invisible"
                          aria-hidden="true"
                        >
                          Data fine
                        </label>
          ...
          >
            ...
            >
              ...
              <div className="p-4 space-y-2">
                <button className="w-full text-center px-4 py-3 text-base font-semibold rounded-lg transition-colors bg-slate-100 text-slate-800 hover:bg-indigo-100 hover:text-indigo-800" onClick={() => handleFrequencySelect('single')}>Singolo</button>
                <button className="w-full text-center px-4 py-3 text-base font-semibold rounded-lg transition-colors bg-slate-100 text-slate-800 hover:bg-indigo-100 hover:text-indigo-800" onClick={() => handleFrequencySelect('recurring')}>Ricorrente</button>
              </div>
            </div>
          </div>
        )}
          ...
          >
            ...
            >
              ...
              <main className="p-4 space-y-4">
                <div className="relative">
                  <button
                    onClick={() => setIsRecurrenceOptionsOpen((p) => !p)}
                    className="w-full flex items-center justify-between text-left gap-2 px-3 py-2.5 text-base rounded-lg border shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors bg-white border-slate-300 text-slate-800 hover:bg-slate-50"
                  >
                    <span className="truncate flex-1 capitalize">
                  ...
                  {isRecurrenceOptionsOpen && (
                    <div className="absolute top-full mt-1 w-full bg-white border border-slate-200 shadow-lg rounded-lg z-10 p-2 space-y-1 animate-fade-in-down">
                      {(Object.keys(recurrenceLabels) as Array<keyof typeof recurrenceLabels>).map(
                        (key) => (
                          <button
                            key={key}
                            onClick={() => {
        ...
        {isRecurrenceEndModalOpen && (
          <div
            className={`absolute inset-0 z-[60] flex justify-center items-center p-4 transition-opacity duration-300 ease-in-out ${
              isRecurrenceEndModalAnimating ? 'opacity-100' : 'opacity-0'
            } bg-slate-900/60 backdrop-blur-sm`}
          ...
          >
            <div
              className={`bg-white rounded-lg shadow-xl w-full max-w-xs transform transition-all duration-300 ease-in-out ${
                isRecurrenceEndModalAnimating ? 'scale-100 opacity-100' : 'scale-95 opacity-0'
              }`}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-between items-center p-4 border-b border-slate-200">
              ...
              <div className="p-4 space-y-2">
                <button
                  onClick={() => handleRecurrenceEndTypeSelect('forever')}
                ...
                <button
                  onClick={() => handleRecurrenceEndTypeSelect('date')}
                  className="w-full text-center px-4 py-3 text-base font-semibold rounded-lg transition-colors bg-slate-100 text-slate-800 hover:bg-indigo-100 hover:text-indigo-800"
                >
                  Fino a
                </button>
                <button
                  onClick={() => handleRecurrenceEndTypeSelect('count')}
                  className="w-full text-center px-4 py-3 text-base font-semibold rounded-lg transition-colors bg-slate-100 text-slate-800 hover:bg-indigo-100 hover:text-indigo-800"
                >
                  Numero di volte
                </button>
